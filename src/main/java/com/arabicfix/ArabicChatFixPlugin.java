...
public class ArabicChatFixPlugin extends JavaPlugin implements Listener {
    ...
    private void setupDiscordSRV() {
        try {
            github.scarsz.discordsrv.DiscordSRV discordSRV = (github.scarsz.discordsrv.DiscordSRV) Bukkit.getPluginManager().getPlugin("DiscordSRV");
            
            discordSRV.getScm().subscribe(new github.scarsz.discordsrv.api.Subscribe() {
                @github.scarsz.discordsrv.api.Subscribe
                public void onDiscordMessage(github.scarsz.discordsrv.api.events.DiscordGuildMessagePostBroadcastEvent event) {
                    String minecraftMessage = event.getMinecraftMessage();
                    
                    if (!containsArabic(minecraftMessage)) {
                        return; // No Arabic, skip
                    }
                    
                    // Discord message with Arabic - need to show differently to Bedrock vs Java
                    // Cancel the default broadcast
                    event.setCancelled(true);
                    
                    String convertedMessage = convertArabicForBedrock(minecraftMessage);
                    
                    // Send to each player based on their client
                    for (Player player : Bukkit.getOnlinePlayers()) {
                        if (floodgateApi.isFloodgatePlayer(player.getUniqueId())) {
                            // Bedrock - show converted
                            player.sendMessage(convertedMessage);
                        } else {
                            // Java - show original
            ...
            getLogger().info("DiscordSRV event listener registered successfully!");
        } catch (Exception e) {
    ...
    public void onPlayerChat(AsyncPlayerChatEvent event) {
        ...
        if (!containsArabic(message)) {
            return; // No Arabic, let it pass normally
        }
        
        // Message has Arabic - we need to show different versions to Bedrock vs Java
        event.setCancelled(true);
        ...
        // Send to in-game players
        for (Player recipient : Bukkit.getOnlinePlayers()) {
            boolean recipientIsBedrock = floodgateApi.isFloodgatePlayer(recipient.getUniqueId());
            String displayMessage;
            
            if (recipientIsBedrock) {
                // Bedrock player ALWAYS sees converted text (so they can read it)
                displayMessage = String.format(format, senderName, convertedMessage);
            } else {
                // Java player ALWAYS sees original text
                displayMessage = String.format(format, senderName, originalMessage);
            }
            
            recipient.sendMessage(displayMessage);
        }
        
        // Send to console (original)
        Bukkit.getConsoleSender().sendMessage(String.format(format, senderName, originalMessage));
        
        // For DiscordSRV: trigger a new event with original message
        // This will be processed by DiscordSRV showing original Arabic
        Bukkit.getScheduler().runTask(this, () -> {
            AsyncPlayerChatEvent discordEvent = new AsyncPlayerChatEvent(
                false, sender, originalMessage, event.getRecipients()
            );
            discordEvent.setFormat(format);
            
            // Call the event for DiscordSRV to pick up
