...
public class ArabicChatFixPlugin extends JavaPlugin implements Listener {
    ...
    public void onPlayerChat(AsyncPlayerChatEvent event) {
        ...
        if (floodgateApi.isFloodgatePlayer(sender.getUniqueId()) && containsArabic(message)) {
            // Bedrock player is typing Arabic - convert it to visual forms
            String convertedMessage = convertArabicForBedrock(message);
            event.setMessage(convertedMessage);
            
            // Now ALL players (Java and Bedrock) will see the converted text
            // And DiscordSRV will also send the converted text to Discord
        }
        
        // If sender is Java player, message stays normal - no conversion needed
    }
